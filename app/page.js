...
export default function Home() {
  ...
  const [editingData, setEditingData] = useState({ customer_name: '', customer_phone: '', customer_address: '', products: [], note: '', status: 'payment_pending' })
  const [newOrder, setNewOrder] = useState({ customer_name: '', customer_phone: '', customer_address: '', products: [{ product: '', quantity: 1, unit_price: '', kdv_rate: 0 }], note: '' })
  ...
  const colors = {
    light: { bg: '#ffffff', bgSecondary: '#f5f7fa', text: '#1a1a1a', textSecondary: '#666666', border: '#e0e4e8', header: '#ffffff', input: '#ffffff', inputBorder: '#d0d5db', hover: '#f9fafb' },
    dark: { bg: '#1a1a1a', bgSecondary: '#2d2d2d', text: '#e0e0e0', textSecondary: '#b0b0b0', border: '#3a3a3a', header: '#252525', input: '#2d2d2d', inputBorder: '#3a3a3a', hover: '#333333' }
  }
  ...
  useEffect(() => {
    let filtered = orders
    if (searchName.trim()) filtered = filtered.filter(o => o.customer_name.toLowerCase().startsWith(searchName.toLowerCase()))
    if (searchPhone.trim()) filtered = filtered.filter(o => o.customer_phone.startsWith(searchPhone))
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      ...
      setResetEmail('')
      setTimeout(() => { setIsForgotPassword(false); setResetMessage('') }, 3000)
    } catch (error) {
  ...
  const calculateKDVAmount = (product) => {
    ...
    return ((quantity * unitPrice * kdvRate) / 100).toFixed(2)
  }
  const calculateSubtotal = (product) => ((parseInt(product.quantity) || 1) * (parseFloat(product.unit_price) || 0)).toFixed(2)
  const calculateLineTotal = (product) => (parseFloat(calculateSubtotal(product)) + parseFloat(calculateKDVAmount(product))).toFixed(2)
  const calculateGrandTotal = () => newOrder.products.reduce((sum, p) => sum + parseFloat(calculateLineTotal(p)), 0).toFixed(2)
  const calculateEditGrandTotal = () => !editingData.products ? '0.00' : editingData.products.reduce((sum, p) => sum + parseFloat(calculateLineTotal(p)), 0).toFixed(2)
  const calculateTotalKDV = () => newOrder.products.reduce((sum, p) => sum + parseFloat(calculateKDVAmount(p)), 0).toFixed(2)
  const calculateEditTotalKDV = () => !editingData.products ? '0.00' : editingData.products.reduce((sum, p) => sum + parseFloat(calculateKDVAmount(p)), 0).toFixed(2)

  const addProductLine = () => setNewOrder({ ...newOrder, products: [...newOrder.products, { product: '', quantity: 1, unit_price: '', kdv_rate: 0 }] })
  const addProductLineEdit = () => setEditingData({ ...editingData, products: [...editingData.products, { product: '', quantity: 1, unit_price: '', kdv_rate: 0 }] })
  const updateProductLine = (index, field, value) => { const p = [...newOrder.products]; p[index] = { ...p[index], [field]: value }; setNewOrder({ ...newOrder, products: p }) }
  const updateProductLineEdit = (index, field, value) => { const p = [...editingData.products]; p[index] = { ...p[index], [field]: value }; setEditingData({ ...editingData, products: p }) }
  const removeProductLine = (index) => setNewOrder({ ...newOrder, products: newOrder.products.filter((_, i) => i !== index) })
  const removeProductLineEdit = (index) => setEditingData({ ...editingData, products: editingData.products.filter((_, i) => i !== index) })

  const parseProducts = (order) => {
    if (order.products_json) { try { return JSON.parse(order.products_json) } catch (e) {} }
    try {
    ...
    } catch (e) { return [{ product: order.product, quantity: 1, unit_price: 0, kdv_rate: 0 }] }
  }

  const startEditing = (order) => {
    setEditingId(order.id)
    setEditingData({ customer_name: order.customer_name, customer_phone: order.customer_phone, customer_address: order.customer_address || '', products: parseProducts(order), note: order.note, status: order.status })
  }
  ...
  const saveEdit = async () => {
    try {
      ...
      const { error } = await supabase.from('orders').update({ customer_name: editingData.customer_name, customer_phone: editingData.customer_phone, product: productList, products_json: JSON.stringify(editingData.products), price: totalPrice, note: editingData.note, status: editingData.status }).eq('id', editingId)
  ...
  const handleAddOrder = async (e) => {
    ...
    try {
      ...
      const { error } = await supabase.from('orders').insert([{ user_id: user.id, customer_name: newOrder.customer_name, customer_phone: newOrder.customer_phone, product: productList, products_json: JSON.stringify(newOrder.products), price: totalPrice, status: 'payment_pending', note: newOrder.note }])
      if (error) throw error
  ...
  const statusColors = { payment_pending: '#ff6b6b', paid: '#ffd93d', preparing: '#6bcf7f', shipped: '#4d96ff', completed: '#95e1d3' }
      ...
      <div style={{ background: c.header, borderBottom: `1px solid ${c.border}`, padding: '15px 20px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
        <div style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
          ...
          <div style={{ display: 'flex', gap: '15px', alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <button onClick={handleLogout} style={{ padding: '8px 15px', background: '#ff6b6b', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold' }}>Çıkış</button>
          </div>
        </div>
      </div>
      ...
      <div style={{ width: '100%', padding: '20px' }}>
        ...
        <div style={{ background: c.header, padding: '15px 20px', borderRadius: '8px', marginBottom: '20px', border: `1px solid ${c.border}` }}>
                  ...
                  <tr key={index} style={{ borderBottom: `1px solid ${c.border}`, background: index % 2 === 0 ? c.header : c.bgSecondary }}>
                    ...
                    <td style={{ padding: '8px', textAlign: 'center' }}>
                      <button type="button" onClick={() => removeProductLine(index)} disabled={newOrder.products.length === 1}
                        style={{ padding: '4px 6px', background: newOrder.products.length === 1 ? '#ccc' : '#dc3545', color: 'white', border: 'none', borderRadius: '4px', cursor: newOrder.products.length === 1 ? 'not-allowed' : 'pointer', fontWeight: 'bold', fontSize: '13px' }}>🗑️</button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <button type="button" onClick={addProductLine}
            style={{ padding: '6px 10px', background: '#28a745', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', fontSize: '13px', marginBottom: '15px' }}>➕</button>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '10px' }}>
            ...
            <div>
              <button onClick={() => { setSearchName(''); setSearchPhone('') }}
                style={{ padding: '8px 15px', background: '#6c757d', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', fontSize: '14px', marginTop: '23px' }}>Temizle</button>
            </div>
          </div>
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
          <div style={{ background: c.header, border: `1px solid ${c.border}`, borderRadius: '8px', padding: '20px', maxWidth: '900px', width: '100%', maxHeight: '90vh', overflowY: 'auto', color: c.text }}>
                    ...
                    <tr key={index} style={{ borderBottom: `1px solid ${c.border}`, background: index % 2 === 0 ? c.header : c.bgSecondary }}>
                      ...
                      <td style={{ padding: '8px', textAlign: 'center' }}>
                        <button onClick={() => removeProductLineEdit(index)} disabled={editingData.products.length === 1}
                          style={{ padding: '4px 6px', background: editingData.products.length === 1 ? '#ccc' : '#dc3545', color: 'white', border: 'none', borderRadius: '4px', cursor: editingData.products.length === 1 ? 'not-allowed' : 'pointer', fontWeight: 'bold', fontSize: '13px' }}>🗑️</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <button onClick={addProductLineEdit}
              style={{ padding: '6px 10px', background: '#28a745', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold', fontSize: '13px', marginBottom: '20px' }}>➕</button>
